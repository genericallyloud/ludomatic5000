...
function newComponent(ludoProject){
    ...
    var stubFiles = ["dataFile","templateFile","styleFile"];
    if(ludoProject.usingLess()){
        stubFiles.push("lessFile");
    }
    
    //now add stub files into each directory
    stubFiles.forEach(function(fileFunc){
...
function drawPdf(ludoProject, server){
    wkhtmltopdf(ludoProject.url(),
        { 
            pageSize: ludoProject.pageSize(),
            orientation:ludoProject.orientation(),
            marginBottom:0,
            marginLeft:0,
            marginTop:0,
            marginRight:0
...
function drawImage(ludoProject, server){
    ...
    ludoProject.prepareCache(function(processed){
        ...
        for(var i=0; i<count; i++){
            //need to wrap in quotes because its going out to the command line
            var outputFile = '"'+path.join(outputDir, ludoProject.component+i+".png")+'"';
            wkhtmltoimage.generate(baseUrl+"/row/"+i,
                {
                    width:300*ludoProject.width(),//dpi * inches wide
                    height:300*ludoProject.height(),//dpi * inches high
