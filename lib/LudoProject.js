...
LudoProject.prototype.compileLess = function(){
    ...
    less.render(lessInput,
        {
            paths: [this.lessDirectory()],  // Specify search paths for @import directives
            filename: this.component + '.less', // Specify a filename, for better error messages
            compress: false          // Minify CSS output
        },
        function(e,output) {
            if(e){
                console.error("Error processing LESS file");
                console.error(e);
                //in the case of design mode, we don't want to just shut it down
                if(this.command === "draw"){
                    process.exit();
                }
            }else{
...
LudoProject.prototype.pageSize = function(){
    return this.finalConfigs.target !== "tgc";
};

LudoProject.prototype.orientation = function(){
    return this.finalConfigs.target !== "tgc";
};
...
LudoProject.prototype.lessFile = function(){
    return path.join(this.lessDirectory(), this.component+".less");
};


LudoProject.prototype.lessDirectory = function(){
    return path.join(this.projectDirectory, "less");
};
...
LudoProject.prototype.fetchCsvData = function(callback){
    var convert = function(csvConverter, data, callback){
        csvConverter.fromString(data, callback);
    };

    var csvConverter = new Converter(this && this.config && this.config.csvParams || {ignoreEmpty:true});
    if(this.usingGoogle()){
        request(this.getGoogleUrl(), function(err, response, body){
            if(err){
                console.error(err);
            }
            convert(csvConverter, body, callback);
        });
    } else{
        convert(csvConverter, fs.readFileSync(this.dataFile()).toString(), callback);
    }
};
